...
// SandboxCapExceededError is returned when the envelope's sum of native payments
// exceeds the configured cap. Callers can use errors.Is() or type assertions to
// distinguish this specific error condition from other failures.
type SandboxCapExceededError struct {
        Actual uint64 // The actual native payment amount in stroops
        Cap    uint64 // The configured cap in stroops
}

func (e SandboxCapExceededError) Error() string {
        return fmt.Sprintf("sandbox native token cap exceeded: total native payments %d stroops exceeds cap %d stroops", e.Actual, e.Cap)
}

func (e SandboxCapExceededError) Unwrap() error {
        return ErrSandboxNativeTokenCapExceeded
}
...
func EnforceSandboxNativeTokenCap(envelopeXdr string, capStroops uint64) error {
        ...
        if sum > capStroops {
                return SandboxCapExceededError{Actual: sum, Cap: capStroops}
        }
        return nil
}
